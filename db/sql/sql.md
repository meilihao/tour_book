...
4. 验证索引
    ...
    原理：允许在创建索引期间，DML（数据修改语言，如INSERT, UPDATE, DELETE）操作仍然可以正常执行。读操作通常也不受影响。
    数据库支持：
        - MySQL 5.6+：InnoDB 存储引擎支持大部分 Online DDL。使用 ALGORITHM=INPLACE 或 ALGORITHM=INSTANT。
            - INPLACE：拷贝表数据，可能涉及重建表，但DML不会被阻塞太久。
            - INSTANT：仅修改元数据，速度极快，且不重建表, 无需加排他MDL锁，原表数据也不受影响. 整个DDL过程几乎是瞬间完成的，也不会阻塞DML。这个新特性是8.0.12引入. 适用于某些特定操作（如添加列、修改列名）。但创建二级索引不属于 INSTANT 范畴。
    ...
    1. 第三方工具（如 pt-online-schema-change for MySQL）
