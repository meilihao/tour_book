...
- [容灾备份系统Datapp-CDP](https://www.datapp.com.cn/product/backup/9.html)
  ...
  使用写时间戳测试时(通过iscsi将zfs volume挂载到client并格式化成xfs, 并发写10个文件, 文件序号是奇数每秒写10条而偶数写100条. cdp时间戳由cdp server(与iscsi server同机)生成), 下面测试数据已忽略纳秒数:
  ...
  测试发现zfs snap view更接近指定时间, 应是受相应实现的影响, 比如fs wal, 落盘策略等等, 因为打印回放的最后几个bio, 其已包含`10:57:23`和`10:57:24`的时间戳
