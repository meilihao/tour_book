...
## 可观察性
参考:
- [OpenObserve 是一个云原生可观测性平台，用Rust编写核心](https://mp.weixin.qq.com/s/NaiaFTpaWKpbxWhGlOSPqw)

  一个二进制文件搞定日志、指标、链路、前端监控、告警、Pipeline 全链路的统一平台
...
- [编译时插桩，Go 应用监控最佳选择](https://observability.cn/article/qzhkmtx16c9r1sdu/)
- [OpenTelemetry 的 Go 编译时插桩正式发布 v1 版本](https://opentelemetry.io/blog/2026/go-compile-time-instrumentation-v1/)
- [Coroot 体验零侵入可观测性](https://github.com/coroot/coroot)
...
## 监控
ref:
- [为什么VictoriaMetrics正在替换Prometheus？一次大规模可观测性迁移实录](https://zhuanlan.zhihu.com/p/1935442875837060515)
...
- [Prometheus vs VictoriaMetrics](https://www.cnblogs.com/waldron/p/17947153)
  ...
  - [Scaling Observability: Why TiDB Moved from Prometheus to VictoriaMetrics](https://www.pingcap.com/blog/tidb-observability-migrating-prometheus-victoriametrics/)
    ...
    -retentionPeriod=10d
    ```
- [InsideVictoriaMetrics](https://github.com/ahfuzhang/InsideVictoriaMetrics)
- [VictoriaMetrics 集群部署](https://www.cnblogs.com/wangguishe/p/18748238)
- [得物云原生全链路追踪Trace2.0架构实践](https://cloud.tencent.com/developer/article/2099189)

  目前业内比较流行的全链路追踪开源项目(SkyWalking、Pinpoint、Jaeger等)采用的存储大都是基于ES或者HBase实现的。而近几年新兴的开源全链路追踪开源项目(Uptrace、Signoz等)采用的存储大都是基于ClickHouse实现的.
...
- [Database backup tool](https://github.com/databasus/databasus)
...
- 经济
  ...
  - MeiliSearch (by rust, **推荐**)
  - Zinc (by go)
## 流媒体
- [重新定义流媒体服务器 : Monibuca](https://www.infoq.cn/article/uiPl8dIuQmhipKb3q3Tz)
- [Media over QUIC（MoQ）是一个新兴的直播媒体标准，旨在取代WebRTC、HLS/DASH，甚至RTMP/SRT，成为统一标准](https://mp.weixin.qq.com/s/FQ5DtVe-my_g9STHonBqqw)
...
- [License Manager 发布首个框架版本 v0.1.0 —— 开源现代化软件授权管理解决方案](https://www.oschina.net/news/366037)
- [高效开源许可管理：Licensecc全方位软件授权解决方案](https://opc.csdn.net/69845f59437a6b40336bf08d.html)
- [Keygen: Software Licensing and Distribution API](https://keygen.sh/)
- [Cryptlex](https://github.com/cryptlex/lexactivator-go)
- [Logto 是为现代应用和 SaaS 产品设计的身份与访问管理 (IAM) 解决方案](https://github.com/logto-io/logto)
- [MLiev IAM 是一个基于 Go 语言开发的现代化企业级身份认证与访问管理（IAM）系统](https://cnb.cool/mliev/iam/mliev-iam)
- [Rust 如何实现许可证管理系统](https://mp.weixin.qq.com/s/8iKJFitk96egUNej_K_J_w)

  https://github.com/alwaysrun/rlicen

  仅参考
- [Teleport：Go 生态的零信任门户](https://mp.weixin.qq.com/s/9aFduqhnHVALSHOyzeNfjw)

  用 Go 写了一个统一的基础设施访问安全平台，把 SSH、Kubernetes、数据库、RDP、甚至 AI Agent 的 MCP 协议全部纳入一套认证、授权和审计体系
...
## NAS
- [Rockstor —— 基于 Btrfs 的现代 NAS 系统，轻量、强大、可自建的开源存储方案](https://mp.weixin.qq.com/s/mmLyw-ca_DvOIcl6vPKBoQ)
- [Truenas Scale]()

## net
- [Tailscale 开源 Tailcat：像 netcat 一样用，加密 + NAT 穿透](https://www.oschina.net/news/502151)