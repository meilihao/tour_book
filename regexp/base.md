...
- `(?=pattern)` : 正预测先行(要匹配的字符串，后面必须紧跟着pattern)。 找到一个匹配项后，将在匹配文本之前开始搜索下一个匹配项。 不会保存匹配项以备将来使用
- `(?!pattern)` : 负预测先行(要求字符串的后面不能紧跟着某个pattern)。 匹配与模式 不匹配的搜索字符串。 找到一个匹配项后，将在匹配文本之前开始搜索下一个匹配项。 不会保存匹配项以备将来使用
