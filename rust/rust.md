...
指向一个特型类型（如 writer）的引用，称为特型目标（trait object）. 特型目标是一个胖指针，包含指向值本身的指针和指向表示该值类型的表(该表在可执行文件中, 一般在 TEXT 或 RODATA 段)的指针, 类似go的`interface{}`. C++ 同样也有这种运行时类型信息，其被称为虚拟表（virtual table）或 vtable.
