...
- PathBuf. 与 Path 配对, Path 的可变版本. PathBuf 包装了 OsString
...
1. 智能指针(smart pointer)
	...
	智能指针实际上是一种结构体, 是对指针的一层封装, 因此它的行为类似指针. 它提供了一些额外的功能, 比如自动释放堆内存.
...
- `Cow<’a, T>` : Clone-on-write, 写时复制指针. 可能是借用指针, 也可能是具有所有权的指针
	...
	Cow<T>的功能是：以不可变的方式访问借用内容，以及在需要可变借用或所有权的时候再克隆一份数据. 它实现了Deref. Cow<T>旨在减少复制操作，提高性能，一般用于读多写少的场景. Cow<T>的另一个用处是统一实现规范.
...
1. 泛型约束: 泛型行为被 trait 限定在更有限的范围内
	...
	impl Trait 实际上是一种较长形式语法的`trait bound`语法糖.
    ...
    对任何满足特定 trait bound 的类型实现 trait 被称为 blanket implementations, 它们被广泛的用于 Rust 标准库中. 例如，标准库为任何实现了 Display trait 的类型实现了 ToString trait: `impl<T: Display> ToString for T {}`.
...
##### 函数签名中的生命周期参数
生命周期参数是为了帮助借用检查器验证非法借用. 函数间传入和返回的借用必须相关联, 并且返回的借用生命周期必须不长于出借方的生命周期.
...
1. 如果只有一个输入生命周期参数（无论是否省略），则该生命周期都将分配给输出生命周期参数
...
> 在 Rust 中，**迭代器和迭代器适配器是 惰性的（lazy）**, 这意味着在调用方法使用迭代器之前它都不会有效果, 仅在需要时对集合中的元素进行求值或访问.
...
Result把错误封装在  `Result<T, E>` 类型中, 同时提供了`?`操作符(即`try!`宏)来传播错误, 方便开发. `Result<T, E>` 类型是一个泛型数据结构，T 代表成功执行返回的结果类型, E 代表错误类型.
