...
Copy 和 Clone 的区别是:
1. Copy是隐式的、廉价的，只适用于栈上的数据
1. Clone是显式的，可能会分配内存，可能很昂贵
...
- RefCell<T>：提供了类型的**内部可变性**，并且不需要实现 Copy 特征. 它用于**运行时**的锁定以确保安全性
    ...
    > 它是在运行时而不是编译时检查借用, 因此即使错误也可编译通过, 但运行会崩溃, 比如使用了2次`borrow_mut()`
...
单元测试每次只隔离测试一个模块：它们规模小，可以测试私有代码.
...
dev-dependencies: 仅用于测试（或示例、基准测试）的依赖项
