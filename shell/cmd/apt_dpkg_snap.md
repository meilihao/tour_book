...
### ubuntu从24.04升级到26.04报`Distribution data outdated`
原因: distro-info-data 不支持这个跳跃版本即Canonical 还没开放正式升级路径

26.04 在刚发布时，通常要等第一个 point release（26.04.1）之后才开放 LTS 升级通道

### apt update时发现sources.list.d/google-chrome.sources被覆盖
Chrome 安装包里包含一个脚本 /etc/cron.daily/google-chrome。每天这个脚本运行，或者每次 Chrome 升级时，它会检查 /etc/apt/sources.list.d/。如果发现文件内容不符合它的“官方标准”，它就会将其重置

解决:
1. `sudo chmod a-x google-chrome`
2. 恢复google-chrome.sources
