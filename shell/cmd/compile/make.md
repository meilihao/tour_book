...
- 变量定义
	...
	`=`: 变量的值是整个Makefile展开后最后被指定的值
	`:=`: 变量的值取决于它在Makefile中的位置而不是整个Makefile展开后的最终值
	...
	- `$+`:与`$^`类似, 但没有排除重复条件
	- `$*`:目标模式中`%`及其之前的部分. 比如获取目标的主文件名, 不包括扩展名
...
deps-arch:
ifeq ($(ARCH), x86_64) # ifeq ifeq相关指令必须定格写
	cp -f app-amd64 app
	$(eval SCRIPTS += app)
