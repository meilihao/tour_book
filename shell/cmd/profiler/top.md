...
第三行(CPU):
1. us: 用户进程的cpu百分比
1. sy: 系统(内核)进程的cpu百分比

    大量的I/O、上下文切换会让这个数字升高
...
1. wa: 等待 I/O 的cpu百分比
    ...
    iowait高只能说明"CPU有过等待I/O"，不能说明磁盘瓶颈的程度。真正看磁盘要用iostat的%util和await
1. 处理硬件中断花费的时间
1. 处理软件中断花费的时间
1. st: 由管理程序hypervisor从这个虚拟机`偷走`的时间，用于其他任务(例如启动另一个虚拟机), 云服务器上这个数字>2%就要注意了
1. gu: 运行虚拟机的时间，物理机上才会看到

CPU使用率的本质是"非idle时间占比"，数据来自 /proc/stat 的 jiffies 计数
CPU 100%时CPU throttling(CPU 节流)才是真正让服务卡的原因
...
## FAQ
### load
Load Average统计的是R+D状态进程数，用指数衰减移动平均算法计算，1min对突发敏感、15min只留"记忆".
Load高不等于CPU高，D状态进程（I/O等待）能撑起很高的Load但CPU很闲.

排查思路:
1. 先看趋势：1min在升还是降？

    1min持续上升 → 问题还在恶化，紧急处理
    1min已开始下降 → 问题可能在消退
 
2. Load/核数 算比值
    <0.7 → 健康，可能只是瞬时波动
...
5. 按方向深入
    磁盘I/O → iostat -x 1 看await/%util
    NFS卡 → 检查NFS服务器状态、网络连通
    Swap → free看available，vmstat看si/so
    CPU计算 → top -H找热点线程
