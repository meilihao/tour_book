...
sed $SedMode "s|$OldRedisPwd|$NewRedisPwd|g" warntask-v2/etc/warntask.yaml
sed $SedMode "s|$OldGcrud|$NewGcrud|g" energy_storage/configs/configs.yaml
```
...
## FAQ
### `sed $SedMode "0,|$OldInfluxDBPwd|s|$OldInfluxDBPwd|$NewInfluxDBPwd|" dee-load-metric/configs/configs.yaml`报错
在 sed 中，0,/regex/ 这种地址范围语法要求正则表达式必须用 / 包裹，不能使用自定义分隔符
