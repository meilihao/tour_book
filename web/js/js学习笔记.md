...
**Origin表示本域**，也就是浏览器当前页面的域. 当JavaScript向外域（如sina.com）发起请求后，**浏览器收到response后，首先检查Access-Control-Allow-Origin是否包含本域,即resp必须有该字段**，如果是，则此次跨域请求成功，如果不是，则请求失败，JavaScript将无法获取到响应的任何数据.
...
```
浏览器确认服务器响应的Access-Control-Allow-Methods头确实包含将要发送的AJAX请求的Method，才会继续发送AJAX，否则，抛出一个错误(见开发者工具的console tag).

由于以POST、PUT方式传送JSON格式的数据在REST中很常见，所以要跨域正确处理POST和PUT请求，服务器端必须正确响应OPTIONS请求.

如果发起请求时设置withCredentials 标志设置为 true，从而向服务器发送cookie， 但是如果服务器端的响应中未携带Access-Control-Allow-Credentials: true，浏览器将不会把响应内容给请求的发送者.
...
func Cors(c *gin.Context) {
    if c.Request.Method != http.MethodOptions {
        ...
        c.Header("Access-Control-Allow-Credentials", "true") // 允许跨域请求携带cookie, 此时Access-Control-Allow-Origin不能"*", 必须是c.Request.Header.Get("Origin")
        c.Next()
    ...
    c.Header("Access-Control-Allow-Methods", "GET, OPTIONS, PATCH, POST, PUT, DELETE")

    c.AbortWithStatus(200)
}
```

不过如果你在a 网域向b 网域的发送请求，且b 网域回传了set-cookie 的讯息，那么浏览器在a 网域会向b 网域的写入一份cookie，但如果请求没有`设定withCredentials(xhr) 或是设定credentials: 'include'(fetch)`的话，就算b的server有回传Set-Cookie，一样不会被写入.
